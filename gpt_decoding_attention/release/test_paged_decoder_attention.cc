...
template<typename T>
bool test_paged_masked_multihead_attention(const test_args_t& test_args)
{
    ...
    TIMEIT(true, 10, stream, paged_masked_multihead_attention, params_T, stream);
...
int main(int argc, char** argv)
{
    if (argc != 7) {
        printf("[ERROR] Usage: %s batch_size head_num max_seq_len"
               ...
               argv[0]);
        printf("e.g., %s 32 16 40 256 32 8\n", argv[0]);
        return EXIT_FAILURE;
    }

    const test_args_t test_args{atoi(argv[1]), atoi(argv[2]), atoi(argv[3]), atoi(argv[4]), atoi(argv[5]), atoi(argv[6])};
    ...
    test_pass = test_paged_masked_multihead_attention<half>(test_args);
    printf("%s\n", test_pass ? "." : "X");
    global_test_pass |= test_pass;

    test_pass = test_paged_masked_multihead_attention<__nv_bfloat16>(test_args);
    printf("%s\n", test_pass ? "." : "X");
}
