...
{
    uint16_t c;
    __asm__ __volatile__("v_add_f16 %0, %1, %2;\n" : "=v"(c) : "v"(a), "v"(b));
    return c;
}
...
inline __device__ uint16_t float_to_half(float f)
{
    ...
    float zero = 0.f;
    __asm__ __volatile__("cvt.rn.f16x2.f32 %0, %1, %2;\n" : "=r"(tmp.u32) : "f"(zero), "f"(f));
#else
    __asm__ __volatile__("v_cvt_f16_f32 %0, %1;\n" : "=v"(tmp.u16[0]) : "v"(f));
#endif
    return tmp.u16[0];
}
...
{
    float f;
    __asm__ __volatile__("v_cvt_f32_f16 %0, %1;\n" : "=v"(f) : "v"(h));
    return f;
}
...
inline __device__ float2 half2_to_float2(uint32_t v)
{
    ...
    tmp.u32 = v;
    return make_float2(half_to_float(tmp.u16[0]), half_to_float(tmp.u16[1]));
}
...
inline __device__ uint32_t h0_h0(uint16_t a)
{
    ...
    //__asm__ __volatile__("v_mov_b32 %0, {%1, %1};" : "=r"(b) : "v"(a));
    //return b;
}
...
{
    uint32_t d;
    __asm__ __volatile__("V_PK_FMA_F16 %0, %1, %2, %3;\n" : "=v"(d) : "v"(a), "v"(b), "v"(c));
    //__fma_rn(a,b,c); 
    return d;
}
...
inline __device__ uint2 fma(uint2 a, int32_t b, uint2 c)
{
    uint2 fd;
    union {
        int32_t int32;
    ...
    fd.x = fma(a.x, float2_to_half2(make_float2(int8[0], int8[1])), c.x);
    fd.y = fma(a.y, float2_to_half2(make_float2(int8[2], int8[3])), c.y);
    return fd;
}

inline __device__ uint4 fma(uint4 a, int64_t b, uint4 c)
{
    uint4 fd;
    union {
        int64_t int64;
    ...
    fd.x = fma(a.x, float2_to_half2(make_float2(int8[0], int8[1])), c.x);
    fd.y = fma(a.y, float2_to_half2(make_float2(int8[2], int8[3])), c.y);
    fd.z = fma(a.z, float2_to_half2(make_float2(int8[4], int8[5])), c.z);
    fd.w = fma(a.w, float2_to_half2(make_float2(int8[6], int8[7])), c.w);

    return fd;
}

inline __device__ Float4_ fma(uint2 a, int32_t b, Float4_ c)
{
    Float4_ fd;
    union {
        int32_t int32;
    ...
    fd.x = fma(a.x, (make_float2(int8[0], int8[1])), c.x);
    fd.y = fma(a.y, (make_float2(int8[2], int8[3])), c.y);
    return fd;
}

inline __device__ Float8_ fma(uint4 a, int64_t b, Float8_ c)
{
    Float8_ fd;
    union {
        int64_t int64;
    ...
    fd.x = fma(a.x, (make_float2(int8[0], int8[1])), c.x);
    fd.y = fma(a.y, (make_float2(int8[2], int8[3])), c.y);
    fd.z = fma(a.z, (make_float2(int8[4], int8[5])), c.z);
    fd.w = fma(a.w, (make_float2(int8[6], int8[7])), c.w);

    return fd;
}

inline __device__ Float4_ fma(uint2 a, Float4_ b, Float4_ c)
{
    Float4_ fd;
...
inline __device__ float2 fma(__nv_bfloat162 a, int16_t b, float2 c)
{
    float2 fd;
    union {
        int16_t int16;
    ...
    fd = fma(a, make_float2(int8[0], int8[1]), c);
    return fd;
}

////////////////////////////////////////////////////////////////////////////////////////////////////

inline __device__ Float4_ fma(bf16_4_t a, int32_t b, Float4_ c)
{
    Float4_ fd;
    union {
        int32_t int32;
...
inline __device__ Float8_ fma(bf16_8_t a, int64_t b, Float8_ c)
{
    Float8_ fd;
    union {
        int64_t int64;
...
inline __device__ float2 fma(float2 a, int16_t b, float2 c)
{
    float2 fd;
    union {
        int16_t int16;
...
inline __device__ Float4_ fma(Float4_ a, int32_t b, Float4_ c)
{
    Float4_ fd;
    union {
        int32_t int32;
...
inline __device__ Float8_ fma(Float8_ a, int64_t b, Float8_ c)
{
    Float8_ fd;
    union {
        int64_t int64;
...
{
    uint16_t c;
    __asm__ __volatile__("v_mul_f16 %0, %1, %2;\n" : "=v"(c) : "v"(a), "v"(b));
    return c;
    //return __hmul(a,b);
}
...
{
    uint32_t c;
    __asm__ __volatile__("v_pk_mul_f16 %0, %1, %2;\n" : "=v"(c) : "v"(a), "v"(b));
    return c;
}
...
inline __device__ __nv_bfloat162 rotary_embedding_transform(const __nv_bfloat162 v, const float2 coef)
{
    ...
    float2 rot_fv = rotary_embedding_transform(fv, coef);
    return __floats2bfloat162_rn(rot_fv);
}
#endif
...
write_smem_transpose(const bf16_4_t& vec, __nv_bfloat16* smem, int transpose_idx, int smem_pitch)
{
    ...
    tmp_3.u32x2  = __builtin_bit_cast(uint2, vec);
...
write_smem_transpose(const bf16_8_t& vec, __nv_bfloat16* smem, int transpose_idx, int smem_pitch)
{
    ...
    tmp_3.u32x4  = __builtin_bit_cast(uint4, vec);
    tmp_1.u16[0] = tmp_3.u16[0];
    tmp_2.u16[0] = tmp_3.u16[1];
    tmp_1.u16[1] = tmp_3.u16[2];
    tmp_2.u16[1] = tmp_3.u16[3];
