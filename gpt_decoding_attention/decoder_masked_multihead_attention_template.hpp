...
template<typename T>
struct Qk_vec_accum_ {
};

template<>
struct Qk_vec_accum_<float> {
    using Type = float;
};
template<>
struct Qk_vec_accum_<float2> {
    using Type = float2;
};
template<>
struct Qk_vec_accum_<float4> {
    using Type = float4;
};
// template<> struct Qk_vec_acum_<uint16_t> { using Type = float;        };
template<>
struct Qk_vec_accum_<uint32_t> {
    using Type = uint32_t;
};
template<>
struct Qk_vec_accum_<uint2> {
    using Type = uint2;
};
template<>
struct Qk_vec_accum_<uint4> {
    using Type = uint4;
};

template<>
struct Qk_vec_accum_<__nv_bfloat16> {
    using Type = float;
};
template<>
struct Qk_vec_accum_<__nv_bfloat162> {
    using Type = float2;
};
template<>
struct Qk_vec_accum_<bf16_4_t> {
    using Type = Float4_;
};
template<>
struct Qk_vec_accum_<bf16_8_t> {
    using Type = Float8_;
};
...
template<>
struct K_vec_acum_<float> {
    using Type = float;
};
template<>
struct K_vec_acum_<float2> {
    using Type = float2;
};
template<>
struct K_vec_acum_<float4> {
    using Type = float4;
};
template<>
struct K_vec_acum_<uint32_t> {
    using Type = uint32_t;
};
template<>
struct K_vec_acum_<uint2> {
    using Type = uint2;
};
template<>
struct K_vec_acum_<uint4> {
    using Type = uint4;
};
template<>
struct K_vec_acum_<__nv_bfloat16> {
    using Type = float;
};
template<>
struct K_vec_acum_<__nv_bfloat162> {
    using Type = float2;
};
template<>
struct K_vec_acum_<bf16_4_t> {
    using Type = Float4_;
};
template<>
...
#else
    using K_vec_acum = typename K_vec_acum_<K_vec>::Type;
#endif
...
{
    dst.x = __floats2bfloat162_rn(src.x);
...
#pragma unroll
            for (int mask = QK_VECS_PER_WARP / 2; mask >= 1; mask /= 2) {
                //qk += __shfl_xor(shfl_mask(QK_VECS_PER_WARP), qk, mask);
                qk += __shfl_xor( qk, mask);
            }
        }
    }
