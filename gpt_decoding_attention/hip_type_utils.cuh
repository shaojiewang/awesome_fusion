...
inline __device__ __nv_bfloat162 ldg(const __nv_bfloat162* val) {
    return __builtin_bit_cast(__nv_bfloat162, __ldg(reinterpret_cast<const int*>(val)));
}

template<>
inline __device__ bf16_4_t ldg(const bf16_4_t* val) {
    // int2 dst;
    // const int2* src = reinterpret_cast<const int2*>(val);
    // asm volatile ("flat_load_dwordx2 %0, %1" : "=v"(dst) : "v"(src) : "memory");
    // return __builtin_bit_cast(bf16_4_t, dst);
    return __builtin_bit_cast(bf16_4_t, __ldg(reinterpret_cast<const int2*>(val)));
}

template<typename T>
inline __device__ T ldg_gl(const T* val) {
    return __ldg(val);
}

template<>
inline __device__ bf16_4_t ldg_gl(const bf16_4_t* val) {
    int2 dst;
    const int2* src = reinterpret_cast<const int2*>(val);
...
template<>
inline __device__ bf16_8_t ldg_gl(const bf16_8_t* val) {
    int4 dst;
    const int4* src = reinterpret_cast<const int4*>(val);
    asm volatile ("global_load_dwordx4 %0, %1, off" : "=v"(dst) : "v"(src) : "memory");
    return __builtin_bit_cast(bf16_8_t, dst);
    // return __builtin_bit_cast(bf16_4_t, __ldg(reinterpret_cast<const int2*>(val)));
}

template<>
inline __device__ bf16_8_t ldg(const bf16_8_t* val) {
    // int4 dst;
    // const int4* src = reinterpret_cast<const int4*>(val);
    // asm volatile ("flat_load_dwordx4 %0, %1" : "=v"(dst) : "v"(src) : "memory");
    // return __builtin_bit_cast(bf16_8_t, dst);
    return __builtin_bit_cast(bf16_8_t, __ldg(reinterpret_cast<const int4*>(val)));
}
...
template<> __device__ inline int8_t cuda_cast<int8_t, half>(half val) {
    ...
    int8[0] = static_cast<int8_t>(fp32);
    return int8[0];
}
...
template<> __device__ inline __nv_bfloat16  cuda_abs(__nv_bfloat16  val) { return bf16abs(val); }
template<> __device__ inline __nv_bfloat162 cuda_abs(__nv_bfloat162 val) { return bf16abs2(val); }
