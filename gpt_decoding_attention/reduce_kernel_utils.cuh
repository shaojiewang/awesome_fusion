...
template<>
__inline__ __device__ __half warpReduceSum(__half val)
{
#pragma unroll
    for (int mask = warpSize / 2; mask > 0; mask >>= 1)
        val = add(val, static_cast<__half>(__shfl_xor( static_cast<float>(val), mask, warpSize)));  //__shfl_sync bf16 return float when sm < 80
    return val;
}
...
#pragma unroll
        for (int mask = 16; mask > 0; mask >>= 1)
            val[i] = max(val[i], __shfl_xor( val[i], mask, 32));
    }
    return (T)(0.0f);
}
