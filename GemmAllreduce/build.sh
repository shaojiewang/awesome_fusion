...
hipcc ${TOP}/syncthreads_or.cu ${TOP}/custom_ar_kernels.cu ${TOP}/custom_ar_comm.cc ${TOP}/nccl.cc -DASM_PRINT=0 -DINT_INIT=1 ${TOP}/${SRC} -I/usr/local/include -pthread -L/usr/local/lib -L /opt/rocm/rccl/lib/ -Wl,-rpath -Wl,/usr/local/lib -Wl,--enable-new-dtags -lmpi -lrccl -lrocblas -std=c++17 -DBUILD_MULTI_GPU=ON --offload-arch=gfx90a -o ${BUILD}/${OUT} --save-temps
